...
#	include "config.h"
#endif
#include "gnu_io_internal_RXTXPort.h"
#endif /* dima */
...
JavaVM *vm = NULL;
JNIEnv *get_java_environment(JavaVM *,  jboolean *);
 ...
 * */
#define RXTXPort(foo) Java_gnu_io_internal_RXTXPort_ ## foo
#define RXTXVersion(foo) Java_gnu_io_internal_RXTXVersion_ ## foo
#define RXTXCommDriver(foo) Java_gnu_io_internal_RXTXCommDriver_ ## foo
...
----------------------------------------------------------*/
int configure_port( int fd, int slip )
{
	...
	if(slip)
	{
		printf("ENTER SLIP MODE CONFIGURATION");
		cfmakeraw(&ttyset);
		ttyset.c_cflag &= ~HUPCL;
		ttyset.c_cflag &= ~CLOCAL;
		ttyset.c_cc[ VMIN ] = 0;
		ttyset.c_cc[ VTIME ] = 0;
		if( cfsetispeed(&ttyset, B115200) < 0 )
		{
			goto fail;
		}
		if( cfsetospeed(&ttyset, B115200) < 0 )
		{
			goto fail;
		}
		ttyset.c_cflag |= CLOCAL;

		if(tcsetattr(fd, TCSAFLUSH, &ttyset) == -1)
		{
	...
	} else
	{
		...
		{
			printf("tcsetattr() result less than 0");
			goto fail;
		}
	}
	#ifndef WIN32
 ...
 * JNIEnv struct
 * */
struct JNIEnv_Observer
{
	jobject obj;
	int hash;
	struct JNIEnv_Observer *next;
};

/**
 * Creates and returns a pointer to a new
 * JNIEnv_Observer struct built using the pointer
 * to the JNIEnv passed as parameter
 */
struct JNIEnv_Observer * new_JNIEnv_Observer(jobject jobj, int hash)
{
	struct JNIEnv_Observer *obs_ptr = malloc((sizeof(int)*2)+sizeof(jobject));
	obs_ptr->next = NULL;
	obs_ptr->hash= hash;
	obs_ptr->obj= jobj;
	return obs_ptr;
...
/**
 * Add the JNIEnv_Observer struct pointer created
 * with the JNIEnv pointer passed as parameter to the
 * list of JNIEnv_Observers whose head is also passed as
 * parameter
 */
struct JNIEnv_Observer * add_JNIEnv_Observer(
		struct JNIEnv_Observer *obs_head,  jobject jobj, int hash)
{
	struct JNIEnv_Observer *obs_ptr = new_JNIEnv_Observer(jobj, hash);
	if(obs_head == NULL)
	{
		obs_head = obs_ptr;
	...
	{
		struct JNIEnv_Observer *current = obs_head;
		while(current->next!=NULL)
		{
			current = current->next;
		}
		current->next=obs_ptr;
	}
	return obs_head;
}

/**
 * Delete the JNIEnv_Observer struct pointer wrapping
 * with the JNIEnv pointer passed as parameter from the
 * list of JNIEnv_Observers whose head is also passed as
 * parameter
 */
struct JNIEnv_Observer * delete_JNIEnv_Observer(
		JNIEnv *env, struct JNIEnv_Observer *obs_head, int hash)
{
	if(obs_head == NULL)
	{
		return NULL;
	...
	{
		struct JNIEnv_Observer *current = obs_head;
		struct JNIEnv_Observer *previous = NULL;
		...
		while(current!=NULL)
		{
			if(current->hash == hash)
			{
				...
				current->next=NULL;
				current->hash=0;
				(*env)->DeleteGlobalRef(env, current->obj);
				free(current);

				printf("[Observer_%i] has been deleted\n",hash);
				break;
			}
			previous = current;
			current = current->next;
...
/**
 * device directory file struct
 */
struct ddf
{
	char *file_name;//the name of the file
	struct ddf *next;//the next ddf
};

/**
 * Creates and returns a pointer to a new
 * ddf struct built using the string
 * name passed as parameter
 */
struct ddf * new_ddf(char *name)
{
	struct ddf *ddf_ptr = malloc(sizeof(int)*2);

	ddf_ptr->next = NULL;

	int length = strlen(name)+1;
	ddf_ptr->file_name = malloc(length * sizeof(char));
	ddf_ptr->file_name[0] = '\0';
	ddf_ptr->file_name = strcat(ddf_ptr->file_name, name);
	return ddf_ptr;
}

/**
 * Frees the ddf struct passed as parameter
 */
void free_ddf(struct ddf *ddf_ptr)
{
	ddf_ptr->next=NULL;
	free(ddf_ptr->file_name);
	free(ddf_ptr);
}

/**
 * Frees each ddf struct of the list
 * whose head pointer is passed as parameter
 */
void free_ddf_list(struct ddf *head)
{
	struct ddf *previous = NULL;
...
/**
 * Adds a new ddf struct
 * as a tail of the list whose head pointer
 * is passed as parameter using the string
 * name also passed as parameter
 */
struct ddf * add_ddf(struct ddf *head, char *name)
{
	struct ddf *listHead = head;
	struct ddf *previous = NULL;
	struct ddf *current = head;
...
/**
 * Copy the list past as parameter and returns a
 * pointer to the new ddf list
 */
struct ddf * copy_ddf_list(struct ddf *ddf_to_copy)
{
	struct ddf *ddf_head = NULL;
	struct ddf *current = ddf_to_copy;

	while(current != NULL)
	{
		add_ddf(ddf_head, current->file_name);
		current = current->next;
	}
	return ddf_head;
}
 ...
 * from the list whose head pointer is
 * also passed as parameter
 */
struct ddf * find_ddf(struct ddf *head, char *name)
{
	struct ddf *current = head;
	while(current != NULL)
	{
...
/**
 * Removes the ddf struct whose string
 * name is passed as parameter from the list whose head
 * pointer is also passed as parameter
 */
struct ddf * remove_ddf(
		struct ddf *head, char *name)
{
	struct ddf * listHead = head;
	struct ddf * previous = NULL;
	struct ddf * current = head;
 ...
 *
 */
struct ddf * calculate_diff(struct ddf *base,
		struct ddf *evaluate, struct ddf *diff)
{
	struct ddf *current_evaluate = evaluate;

	while(current_evaluate != NULL)
	{
		struct ddf *current_base = base;
		while(current_base != NULL)
		{
			if(strcmp(current_evaluate->file_name,current_base->file_name)==0)
			{
				goto next;
			}
			current_base = current_base->next;
		}
		diff = add_ddf(diff, current_evaluate->file_name);
		next:
		current_evaluate = current_evaluate->next;
	}
	return diff;
}

/**
 * Returns the number of ddf struct contained by
 * the list whose head pointer is passed as parameter
 */
int count (struct ddf *head)
{
	struct ddf * current = head;
	int count = 0;
	while(current != NULL)
...
/**
 * print the list whose head pointer is passed as parameter
 */
void print_ddf_list(
		struct ddf *head)
{
...
static struct JNIEnv_Observer *observers = NULL;
...
void sendStringArray(JNIEnv *env, jmethodID method, struct ddf *list)
{
	int filesCount;
	...
	if(filesCount > 0)
	{
		jobjectArray ret = (jobjectArray)(*env)->NewObjectArray(
			env, filesCount, (*env)->FindClass(env, "java/lang/String"), 
			(*env)->NewStringUTF(env, ""));

		int n=0;
		struct ddf *ddf_ptr = list;

		while(ddf_ptr != NULL)
		{
			(*env)->SetObjectArrayElement(env, ret, n++,
				(*env)->NewStringUTF(env, ddf_ptr->file_name));

			ddf_ptr = ddf_ptr->next;
		}
		struct JNIEnv_Observer *observer = observers;
		if(observer == NULL)
		{
			printf("No observer found : 'sendStringArray' method exiting\n");
			return;
		}
		while(observer != NULL)
		{			
			(*env)->CallVoidMethod(env, observer->obj, method, ret);
...
struct ddf * createList(struct ddf *current)
{
	char *path = NULL;

	#ifdef WIN32
		//handle only the first fifty com ports
		char *fixedPath = "\\\\.\\COM";
		char *portName = "COM";
		...
		{
			path = malloc(strlen(fixedPath)+(index>9?2:1)+1);
			sprintf(path,"%s%i",fixedPath,index);

			//just handle the first fifty ports
			HANDLE hComm;
			...
			free(path);
			path = malloc(strlen(portName)+(index>9?2:1)+1);
			sprintf(path,"%s%i",portName,index);

			if (hComm == INVALID_HANDLE_VALUE)
			{
				if (GetLastError() != ERROR_FILE_NOT_FOUND)
				{
					current = add_ddf(current, path);
				}
			} else
			{
				current = add_ddf(current, path);
		...
		if (d)
		{
			...
			{
				path = malloc(strlen(DEVICEDIR)+strlen(dir->d_name)+2);
				sprintf(path,"%s/%s",DEVICEDIR,dir->d_name);
				if( stat(path, &s) == 0 &&  !(s.st_mode & S_IFDIR))
				{
				  current = add_ddf(current, dir->d_name);
				}
				free(path);
			}
			closedir(d);
...
observe
   ...
   accept:
   perform:     observe the referenced directory searching for appeared or
   	   	   	    diseappared files
   return:
   exceptions:  none
   comments:
----------------------------------------------------------*/
void observation_thread(void *arg)
{
	jmethodID appearance;
	jmethodID disappearance;
	jmethodID ended;

	JNIEnv* env = (JNIEnv *)arg;

	jobject jcls = (*env)->FindClass(env, "gnu/io/internal/RXTXCommDriver");

	appearance = (*env)->GetMethodID(env, jcls,"appearance",
			"([Ljava/lang/String;)V");
	disappearance = (*env)->GetMethodID(env, jcls, "disappearance",
			"([Ljava/lang/String;)V");
	ended = (*env)->GetStaticMethodID(env, jcls, "observationThreadStopped",
			"()V");
	...
	struct ddf *list = NULL;

	while(1)
	{
		struct ddf *current = NULL;
		struct ddf *appearance_list= NULL;
		struct ddf *disappearance_list= NULL;
		...
		appearance_list = calculate_diff(list, current, appearance_list);
		disappearance_list = calculate_diff(current, list, disappearance_list);

		struct ddf *diffs[] = {appearance_list, disappearance_list};
		int index = 0;
		int length = 2;
		...
		{
			sendStringArray(env,
				index==0?appearance:disappearance,
					diffs[index]);
		}
		...
		free_ddf_list(appearance_list);
		free_ddf_list(disappearance_list);
		free_ddf_list(list);
		list = current;
	}
	end:
	(*env)->CallStaticVoidMethod(env, jcls, ended, NULL);
	free_ddf_list(list);
}
/*----------------------------------------------------------
observe
   ...
   comments:
----------------------------------------------------------*/
JNIEXPORT void JNICALL RXTXCommDriver(observe)(JNIEnv *env, jclass jcls)
{
	observation_thread((void *)env);
}
...
unobserve
   ...
   accept:
   perform:     stop the observance
   return:
   exceptions:  none
   comments:
----------------------------------------------------------*/
JNIEXPORT void JNICALL RXTXCommDriver(unobserve)(JNIEnv *env, jclass jcls)
{
	pthread_mutex_lock(&observation_mutex);
	observation = 0;
	pthread_mutex_unlock(&observation_mutex);
...
observe
   ...
   comments:
----------------------------------------------------------*/
JNIEXPORT void JNICALL RXTXCommDriver(registerObserver)(JNIEnv *env, jobject jobj)
{
	pthread_mutex_lock(&observer_mutex);
	jclass objectclass = (*env)->FindClass(env, "java/lang/Object");
	jmethodID hashcode = (*env)->GetMethodID(env, objectclass,"hashCode","()I");
	int hash = (*env)->CallIntMethod(env, jobj, hashcode);
	jobject globalRef = (*env)->NewGlobalRef(env, jobj);
	observers = add_JNIEnv_Observer(observers, globalRef, hash);
	pthread_mutex_unlock(&observer_mutex);
}
...
observe
   ...
   comments:
----------------------------------------------------------*/
JNIEXPORT void JNICALL RXTXCommDriver(unregisterObserver)(JNIEnv *env, jobject jobj)
{
	pthread_mutex_lock(&observer_mutex);
	jclass objectclass = (*env)->FindClass(env, "java/lang/Object");
	jmethodID hashcode = (*env)->GetMethodID(env, objectclass,"hashCode","()I");
	int hash = (*env)->CallIntMethod(env, jobj, hashcode);
	observers = delete_JNIEnv_Observer(env, observers, hash);
	pthread_mutex_unlock(&observer_mutex);
}
	...
	jboolean slip)
{
	...
	*/
	...
	if( configure_port( fd, (slip==JNI_TRUE?1:0)))
	{
...
/*----------------------------------------------------------
RXTXCommDriver.testExists

   accept:      tty_name The device to be tested
   perform:     test if the file exists
   return:      JNI_TRUE if the file exists
   exceptions:  none
   comments:
----------------------------------------------------------*/

JNIEXPORT jboolean  JNICALL RXTXCommDriver(testExists)(
	JNIEnv *env,
	jobject jobj,
	jstring tty_name
)
...
/*----------------------------------------------------------
 registerKnownPorts
   ...
   return:      array of names of already known port
   exceptions:  none
   comments:
----------------------------------------------------------*/
JNIEXPORT jobjectArray JNICALL RXTXCommDriver(registerKnownPorts)(JNIEnv *env,
	...
	switch(portType)
	{
		case PORT_TYPE_SERIAL:
			        ...
			        ( ! IOIteratorIsValid( theSerialIterator)))
			    {
					...
					ret= (jobjectArray)(*env)->NewObjectArray(env, 0,
			    ...
			    }else
			    {
					...
					{
						/* begin dima */
						tempStrings[numPorts++] = getRegistryString(theObject, kIODialinDeviceKey);
						tempStrings[numPorts++] = getRegistryString(theObject, kIOCalloutDeviceKey);
						/* end dima */
					}
					ret = (jobjectArray)(*env)->NewObjectArray(env,numPorts,
							(*env)->FindClass(env, "java/lang/String"),
							(*env)->NewStringUTF(env, ""));
					int count;
					for(count = 0;count < numPorts; count++)
					{
						(*env)->SetObjectArrayElement(env, ret, count,
								(*env)->NewStringUTF(env, tempStrings[count]));
					}
			    }
			#else
				ret = (jobjectArray)(*env)->NewObjectArray(env, 0,
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *java_vm, void *reserved)
{
	...
	return JNI_VERSION_1_6;  /* JNI API used */
}
