...
public class RXTXPort extends SerialPort
{
	...
	*  @see gnu.io.SerialPort
	*/
	public RXTXPort( CommPortIdentifier identifier ,boolean slip ) throws PortInUseException
	{
		if (debug)
		{
		...
		*/
		...
		System.out.println("RXTXPort:RXTXPort("+ name+") returns with fd = " + fd);
		
		if (debug)
		{
			z.reportln( "RXTXPort:RXTXPort("+
					name+") returns with fd = " + fd);
		}
	}
	
	private native synchronized int open( String name, boolean slip )throws PortInUseException;
...
//	}
	...
	*/
		public void write( byte b[] ) throws IOException
		{
			...
			try {
				waitForTheNativeCodeSilly();
				System.out.println("JAVA WRITE ARRAY " + Arrays.toString(b));
...
//			System.out.println("==========> EVENT LOOP ENDING " + 
//			RXTXPort.this.name + "[monThreadisInterrupted : "+ RXTXPort.this.monThreadisInterrupted 
//			+ "] <=================");
			
//			CommPortIdentifiers identifiers = CommPortIdentifiers.getInstance(
//					RXTXPort.this.identifier.getPortType());
			
//			if(!RXTXPort.this.monThreadisInterrupted && !identifiers.checkValid(
//					RXTXPort.this.identifier.getPortName()))
//			{
				//identifiers.remove(RXTXPort.this.identifier);
				RXTXPort.this.identifier = null;
//			}			
			if (debug)
			{
	...
	*  @deprecated deprecated but usedeventLoop in Kaffe 
	*/
...
/*------------------------  END OF CommAPI -----------------------------*/
	...
	*/
	...
	{
		return CommPortIdentifiers.PORT_SERIAL;
	}
